...
| **Ledger** | Promoted rulings become durable pair rules. The replayer rescans historical cases. Not a HOLD archive. When new mail arrives, matching SI/BL writings replay those rules so the same pair is not re-Piloted. Header CLEAR/HOLD/PILOT counts come from Postgres (pipeline stamps + human stamps + replay). Refresh reloads that ledger; it does not wipe it. |
